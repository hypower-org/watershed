...
(defn start
  ...
  (let [rs (if (empty? requires) '() (w/outline :getting [(first requires)] (fn [stream] (s/consume #(println "getting: " %) (s/map identity stream)))))
        ps (if (empty? provides) '() (w/outline (first provides) [] (fn [] (s/periodically 1000 (fn [] :system-one)))))]
